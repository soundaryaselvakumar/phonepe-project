...
user_device = pd.read_sql(query5, con=connection)
query6 = 'select * from agg_userbydevice_table'
app_opening = pd.read_sql(query6, con=connection)
...
districts_final = pd.merge(districts, districts_tran, how='outer', on= 'State')
...
with st.container():
    ...
    st.image(r'D:\soundarya\Data-Vizualisation.png')
    st.write(' ')
    st.subheader(
    ...
    scatter_register = px.scatter(scatter_reg_df, x="State", y="Registered_Users_Count",  color="State",
                                  hover_name="State", hover_data=['Year', 'Quarter', 'Brand_Name'], size_max=60)
...
# ------------------------------------------- Geo-analysis ----------------------------------------------------------------------------------
with geo_analysis:
    st.subheader(':Green[Transaction analysis->State and Districtwise:]')
    st.write(' ')
...
fig1 = px.scatter_geo(plot_district,
                          ...
                          color=plot_district['Total_Amount'],
                          size=plot_district['Total_Transactions_count'],
                          hover_name="District",
                          hover_data=["State", 'Total_Amount','Total_Transactions_count', 'Year', 'Quarter'],
                          title='District',
                          size_max=22)
...
fig2 = px.scatter_geo(plot_state_total,
                          ...
                          hover_data=['Transacion_count',
                                      'Transacion_amount', 'Transacion_type','Year', 'Quater'],
                          )
...
with Device_analysis:
    ...
    user_device_treemap = app_opening [(app_opening ['State'] == tree_map_state) & (app_opening ['Year'] == tree_map_state_year) &
                                      (app_opening ['Quarter'] == tree_map_state_quater)]
    user_device_treemap['Brand_Name'] = user_device_treemap['Brand_Name'].astype(
        str)
...
user_device_treemap_fig = px.treemap(user_device_treemap, path=['State', 'Brand_Name'], values='Percentage_Share_of_Brand', hover_data=['Year', 'Quarter'],
                                         color='Brand_Name',
...
bar_user = px.bar(user_device_treemap, x='Brand_Name', y='Registered_Users_Count', color='Brand_Name',
...
with st.sidebar:
    ...
    overall_device = user_device_overall.groupby(['Brand_Name', 'Year']).sum()
    overall_device.reset_index(inplace=True)

    overall_dev_fig = px.bar(overall_device, x='Year', y='Registered_Users_Count',
                             color='Brand_Name', title='Customer Device pattern from 2018 - 2022')
    ...
    overall_reg = overall_reg.groupby(['state']).sum()
    overall_reg.reset_index(inplace=True)

    overall_reg = px.bar(overall_reg, x=['state','year'], y=['Registered_user', 'App_opening'], barmode='group', title='Phonepe installation from 2018 - 2022')
